                    ...
                    <div class="modal-body custom-modal-body">
                        <form>
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="nama_olahraga">
                            </div>
                            <div class="mb-0">
                            ...
                            <div class="modal-footer-btn">
                                <button type="button" class="btn btn-cancel me-2" data-bs-dismiss="modal">Selesai</button>
                                <button type="button" class="btn btn-submit" onClick="submitCabor()">Submit</button>
                            </div>
                        </form>
                    </div>
                </div>
...
<script>
window.submitCabor = function() {
    const namaCabor = $('#nama_olahraga').val().trim();
    const statusCabor = $('#status').prop('checked') ? 1 : 0;
    ...
    if (namaCabor === '') {
        Swal.fire({
            icon: 'warning',
            title: 'Oops...',
            text: 'Nama Olahraga tidak boleh kosong!',
        });
        return; // Hentikan proses jika validasi gagal
    }

    $.ajax({
        url: '/api/sports', // URL untuk mengirim data
        method: 'POST',
        data: formData,
        success: function(response) {
            Swal.fire({
                title: "Berhasil!",
                text: "Data Cabang Olahraga Sudah Disimpan",
                icon: "success",
